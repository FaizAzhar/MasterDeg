...
marg.mod2X.ll <- function(xi,par){
  ...
  n <- length(xi)
  logll <- (-n/2)*(2*pi*par[2]**2)-sum((xi-par[1])**2)/(2*par[2]**2)
  return(-logll)
}
# =======================================
...
sim.run <- function(nrun, sample_size=100, mu, sigma){
  ...
  for(run in 1:nrun){
    ...
    u.i <- dexp(boot.data[,1], rate=marg.mod1.par[1]) # use estimated marginal density to estimate tau
    v.i <- dexp(boot.data[,2], rate=marg.mod1.par[2])
    tau <- TauMatrix(as.matrix(cbind(u.i,v.i)))
    cop.mod1.fit.par <- BiCopTau2Par(family=1,tau=tau[1,2])
    ...
    u.i <- dnorm(boot.data[,1], mean=marg.mod2X.fit.par$par[1], sd=marg.mod2X.fit.par$par[2]) # use estimated marginal density to estimate tau
    v.i <- dexp(boot.data[,2], rate=marg.mod2.par[3])
    tau <- TauMatrix(as.matrix(cbind(u.i,v.i)))
    cop.mod2.fit.par <- BiCopTau2Par(family=1,tau=tau[1,2])
    ...
    #                             lower=0, upper=999)
    # marg.mod1.par <- c(marg.mod1X.fit.par$par,marg.mod1T.fit.par$par)
    # cop.mod1.fit.par <- BiCopEst(pobs(boot.data[,1]),pobs(boot.data[,2]),
    #                              family=1, se=T, method='mle')
    # cop.mod1.par <- cop.mod1.fit.par$par
    # 
    # #    storing all parameters for each run
    # mod1.par.df <- rbind(mod1.par.df,c(haz.mod1.par[1],haz.mod1.par[2],
    #                                    marg.mod1.par[1],marg.mod1.par[2],
  ...
  colnames(mod1.par.df) <- c('haz.par1','haz.par2','marg.par1','marg.par2','cop.par1')
  colnames(mod2.par.df) <- c('haz.par1','haz.par2','haz.par3','marg.par1','marg.par2','marg.par3','cop.par1')
  ...
  return(list('expo-expo'=mod1,'normal-expo'=mod2))
}
