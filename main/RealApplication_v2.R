pkgs <- c('openxlsx','copula','R2OpenBUGS','R2jags','coda','MCMCvis','readxl','survivalROC',
          'DescTools','dplyr','ggplot2','gridExtra','grid','devtools','latex2exp', 'timeROC')
...
ipcwROC.auc <- data.frame('time'=t,'AUC'=timeROC(mayo$t, delta=mayo$status, marker=mayo$x, times=t, cause=1)$AUC)
ipcwROC.auc$model <- 'IPCW'

# ROC curve
p1 <- ggplot(PH.roc[which(PH.roc$time == t[1] | PH.roc$time == t[3] | PH.roc$time == t[5]),], aes(x=1-specificity, y=sensitivity, linetype=as.factor(time))) +
...
p2 <- ggplot(COP.roc[which(COP.roc$time == t[1] | COP.roc$time == t[3] | COP.roc$time == t[5]),], aes(x=1-specificity, y=sensitivity, linetype=as.factor(round(time,2)))) +
  geom_line(linewidth=0.8) +
  scale_linetype_manual(values=c("dotdash","dotted","solid")) +
  xlim(0,1) +
  ylim(0,1) +
  geom_abline(slope=1, linetype='dashed') +
  labs(y=element_blank(), title='Copula Function', lty='Time (years):') +
  theme(axis.text.y=element_blank(),legend.position=c(.7,0.2), legend.direction='vertical')

grid.arrange(p1,p2, nrow=1, ncol=2,
             top = textGrob("Time-dependent ROC curve", gp=gpar(fontsize=20, font=3)))

# AUC trend
AUC.res <- rbind(PH.auc,COP.auc,KM.auc, ipcwROC.auc)
ggplot(AUC.res, aes(x=time, y=AUC)) +
  ...
  theme(legend.position=c(.1,0.3), legend.direction='vertical')
